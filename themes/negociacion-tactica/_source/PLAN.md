...
> **Superseded paths (2026-09-21):** folders are now `leadership-session` / `coaching-session`; participant URLs use `/negociacion-tactica/…`. See repo `AUTHORING.md`. This PLAN remains historical for Grupo B packaging decisions.
...
```
themes/negociacion-tactica/
  group-b/s1-leadership/   # published (/gty/ symlink)
  ├── index.html
  ├── facilitador.html
  ├── role-play.html
  ├── deck/session-01.html
  └── qr/
  group-a/s2-skills/coaching/
  ├── facilitador.html
  ├── deck/session.html
  └── principlesyou/
  _source/                 # briefs (not deployed)
```

Legacy short URL: `/gty/` → `group-b/s1-leadership/`.
